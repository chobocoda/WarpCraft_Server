    ...
    implements ICommandListener
{
    ...
    public void kickPlayer(String s)
    {
        ...
        mcServer.configManager.sendPacketToAllPlayers(new Packet3Chat((new StringBuilder()).append("\2474").append(playerEntity.username).append(" was removed from the game.").toString()));
    ...
    public void handleFlying(Packet10Flying packet10flying)
    {
        ...
        if(hasMoved)
        {
            ...
            if(packet10flying.moving)
            {
                ...
                if((!playerEntity.isPlayerSleeping() && (d10 > 1.6499999999999999D || d10 < 0.10000000000000001D)) || (Math.abs(packet10flying.xPosition) > 32000000D || Math.abs(packet10flying.zPosition) > 32000000D))
                {
                    kickPlayer("warpcraft.disconnect.cheating");
                    logger.warning((new StringBuilder()).append(playerEntity.username).append(" had an illegal stance/position: ").append(d10).toString());
                    return;
                }
            }
            if(packet10flying.rotating)
    ...
    public void handleErrorMessage(String s, Object aobj[])
    {
        ...
        mcServer.configManager.sendPacketToAllPlayers(new Packet3Chat((new StringBuilder()).append("\247c").append(playerEntity.username).append(" has left the game.").toString()));
        mcServer.configManager.playerLoggedOut(playerEntity);
        connectionClosed = true;
    }
    ...
    public void registerPacket(Packet packet)
    {
        ...
        kickPlayer("warpcraft.disconnect.packeterror");
    }
    ...
    public void handleChat(Packet3Chat packet3chat)
    {
        ...
        if(s.length() > 100)
        {
            kickPlayer("warpcraft.disconnect.chaterror");
            return;
        }
        s = s.trim();
